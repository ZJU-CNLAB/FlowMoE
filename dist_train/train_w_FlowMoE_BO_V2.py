...
def comm_thread_fn(dist_world_size, grad_chunk):
    """
    ...
    work.wait()
    # dist.all_reduce(grad_chunk, op=dist.ReduceOp.SUM, async_op=False)
...
def backward_hook(module, grad_in, grad_out, grad_queue):
    ...
    for i in range(num_chunks):
        ...
        Train_comm_thread = threading.Thread(
            ...
            args=(dist_world_size, grad_chunk),
            daemon=True
        )
        all_reduce_threads_group.append(Train_comm_thread)
...
def train(
    ...
    # grad_block_size_history[i], speed_history[i]
    max_history_size = 9
    ...
    for thread in all_reduce_threads_group:
        thread.join()
    ...
    for i in range(n_bo_iter):
        ...
        speed_history.append(((time.time() - BO_start_time) / n_train_steps_each))

    grad_block_size_history = np.array(grad_block_size_history)
    speed_history = np.array(speed_history)

    if dist_rank == 0:
        print("grad_block_size_history:", grad_block_size_history)
        print("speed_history:", speed_history)

    if dist_rank == 0:
        inter_func = interp1d(grad_block_size_history, speed_history, kind='cubic')

        # inter_func = Rbf(grad_block_size_history, speed_history, function='gaussian')
        ...
        result = gp_minimize(
            ...
            acq_func="EI",
            acq_optimizer="lbfgs",
            x0=grad_block_size_history[:, None].tolist(),
            y0=speed_history.tolist(),
            n_calls=25,
